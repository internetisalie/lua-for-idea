...
import com.sylvanaar.idea.Lua.lang.psi.impl.TypesUtil;
...
public class ReachingDefinitionsCollector {
  ...
  public static FragmentVariableInfos obtainVariableFlowInformation(final LuaStatementElement first, final LuaStatementElement last) {
    LuaPsiElement context = PsiTreeUtil.getParentOfType(first, LuaFunctionDefinitionStatement.class, LuaFunctionDefinitionStatement.class, LuaPsiFileBase.class);
    LuaBlock flowOwner;
    if (context instanceof LuaFunctionDefinitionStatement) flowOwner = ((LuaFunctionDefinitionStatement) context).getBlock();
    else flowOwner = null;
    ...
    final VariableInfo[] iarr = null;//filterNonlocals(imap, first);
    final VariableInfo[] oarr = null;//filterNonlocals(omap, first);
  ...
  private static void addClosureUsages(final Map<String, VariableInfo> imap, final Map<String, VariableInfo> omap, final LuaStatementElement first, final LuaStatementElement last, LuaBlock flowOwner) {
    flowOwner.accept(new LuaRecursiveElementVisitor() {
      ...
      private void addUsagesInClosure(final Map<String, VariableInfo> imap, final Map<String, VariableInfo> omap, final LuaFunctionDefinitionStatement closure, final LuaStatementElement first, final LuaStatementElement last) {
          ...
          public void visitReferenceExpression(LuaReferenceExpression refExpr) {
            if (!refExpr.isQualified()) {
              ...
              if (resolved instanceof LuaVariable) {
                LuaIdentifier variable = (LuaVariable) resolved;
                    ...
                    if (!(variable.isField())) {
                      if (!isInFragment(first, last, resolved)) {
                        if (isInFragment(first, last, closure)) {
                          addVariable(name, imap, variable.getManager(), variable.getLuaType());
                        }
                      } else {
                        if (!isInFragment(first, last, closure)) {
                          addVariable(name, omap, variable.getManager(), variable.getLuaType());
                        }
                      }
                    }
                  }
  ...
  private static void addVariable(String name, Map<String, VariableInfo> map, PsiManager manager, LuaPsiType type) {
  ...
  @Nullable
  private static LuaPsiType getType(PsiElement element) {
    return LuaPsiType.VOID;
//    if (element instanceof LuaVariable) return ((LuaVariable) element).getLuaType();
//    else if (element instanceof LuaReferenceExpression) return ((LuaReferenceExpression) element).getLuaType();
//    return null;
  }
