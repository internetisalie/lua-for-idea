...
*/
package com.sylvanaar.idea.Lua.editor.smartEnter;
...
import com.sylvanaar.idea.Lua.editor.smartEnter.fixers.LuaFixer;
import com.sylvanaar.idea.Lua.editor.smartEnter.fixers.StringLiteralFixer;
import com.sylvanaar.idea.Lua.editor.smartEnter.processors.LuaPlainEnterProcessor;
...
public class LuaSmartEnterProcessor extends SmartEnterProcessor {
  ...
  private static final LuaFixer[] ourFixers;
  private static final EnterProcessor[] ourEnterProcessors;

  static {
    final List<LuaFixer> fixers = new ArrayList<LuaFixer>();
    fixers.add(new StringLiteralFixer());
...
//
    ourFixers = fixers.toArray(new LuaFixer[fixers.size()]);

    List<EnterProcessor> processors = new ArrayList<EnterProcessor>();
    processors.add(new LuaPlainEnterProcessor());
...
//    }
    ...
    for (PsiElement child : children) {
      if (atCaret instanceof LuaStatementElement && child instanceof LuaStatementElement) continue;
      collectAllElements(child, res, recurse);
    }
  }

  private static boolean doNotStepInto(PsiElement element) {
    return element instanceof LuaBlock || element instanceof LuaStatementElement;
  }
  ...
  protected PsiElement getStatementAtCaret(Editor editor, PsiFile psiFile) {
    ...
    final LuaBlock codeBlock = PsiTreeUtil.getParentOfType(atCaret, LuaBlock.class, false, LuaStatementElement.class);
    if (codeBlock != null) {
      for (LuaStatementElement statement : codeBlock.getLuaStatements()) {
    ...
    LuaStatementElement controlStatement = PsiTreeUtil.getParentOfType(statementAtCaret, LuaStatementElement.class, false);
  ...
  protected void moveCaretInsideBracesIfAny(@NotNull final Editor editor, @NotNull final PsiFile file) throws IncorrectOperationException {
    ...
    if (CharArrayUtil.regionMatches(chars, caretOffset - "{}".length(), "{}") ||
      ...
      PsiElement elt = PsiTreeUtil.getParentOfType(file.findElementAt(caretOffset - 1), LuaBlock.class);
      reformat(elt);
